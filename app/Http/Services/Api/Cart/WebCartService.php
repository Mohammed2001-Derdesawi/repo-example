        ...
        if ($cartItems && count($cartItems) != 0 && !$cartItems->where('coupon_id', '<>', null)->first() && $user->cart->coupon_id == null) {
            $coupon = $this->couponRepository->findByFilters(
                function ($query) use ($item) {
                    $query->whereJsonContains('subscribe_courses',[(string)$item->cartable_id])->orwhere('basic_subscribe_course_id',$item->cartable_id)->active()->where('is_auto_discount',true);
                }
            );
            if($coupon && $this->couponRepository->isValid($coupon->coupon) && $this->couponRepository->stillUsable($coupon->coupon) && !$coupon->mobile_only){
